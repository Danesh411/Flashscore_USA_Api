def football_extract(event_url, data_elements):
    import requests
    ...
    for data_element in data_elements:
        ...
        # Get all valid match rows (exclude static headers)
        ...
        for match_idx in range(len(match_rows)):
            ...
            # Extract match-specific Product Page ID
            ...
            # Team names (use .get() since it's a single element)
            home_team = match_rows[match_idx].xpath('.//div[contains(@class, "participant--home")]//text()').getall()
            away_team = match_rows[match_idx].xpath('.//div[contains(@class, "participant--away")]//text()').getall()
            ...
            home_team_inning_score = match_rows[match_idx].xpath('.//div[contains(@class,"event__part event__part--home event__part--")]/text()').getall()
            away_team_inning_score = match_rows[match_idx].xpath('.//div[contains(@class,"event__part event__part--away event__part--")]/text()').getall()
            ...
            # Build partial result (venue will be filled later)
            result_dict.update({
                "link": event_url,
                "sport": "football",
                ...
                "team_1_Inning/quater_wise_score": " ".join(home_team_inning_score) if home_team_inning_score else None,
                "Team 2 Inning/quater wise score": " ".join(away_team_inning_score) if away_team_inning_score else None,
